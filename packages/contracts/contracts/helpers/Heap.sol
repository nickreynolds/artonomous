...
library Heap{ // default max-heap
  ...
  function insert(Data storage self, uint priority, uint id) internal returns(Node){//√
  ...
  function extractById(Data storage self, uint id) internal returns(Node){//√
  ...
  function getById(Data storage self, uint id) internal view returns(Node){
  ...
  function getMaxId(Data storage self) internal view returns(uint256) {
    return getMax(self).id;
  }
